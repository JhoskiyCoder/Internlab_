...
def student_application_list(request):
    ...
    return render(
        request,
        "applications/student_application_list.html",
...
def student_application_create(request, vacancy_pk):
    ...
    vacancy = get_object_or_404(
        Vacancy.objects.select_related("employer"), pk=vacancy_pk
    )
...
def employer_application_list(request):
    ...
    status_tabs = [
        {"value": "all", "label": "Все", "count": base_applications.count()},
        {
            ...
            "label": "Новые",
            "count": base_applications.filter(
    ...
    for application in applications.prefetch_related("vacancy__required_skills__skill"):
        ...
        application_cards.append(
            {"application": application, "match_score": match_result.score}
        )
    return render(
        request,
...
def employer_application_detail(request, pk):
    ...
    application = get_object_or_404(
        ...
        ).prefetch_related("student__skills__skill", "vacancy__required_skills__skill"),
        pk=pk,
        vacancy__employer=profile,
    )
    candidate_skills = application.student.skills.select_related("skill").order_by(
        "skill__category", "skill__name"
    )
    candidate_projects = StudentProject.objects.filter(
    ...
    average_skill_level = (
        round(
            sum((item.level for item in candidate_skills)) / candidate_skills.count(), 1
        )
