...
class MatchingServiceTests(TestCase):
    ...
    def test_exact_match_returns_100(self):
        ...
        StudentSkill.objects.create(
            student_profile=self.student_profile, skill=self.python, level=3
        )
        StudentSkill.objects.create(
            student_profile=self.student_profile, skill=self.sql, level=2
        )
    ...
    def test_partial_match(self):
        ...
        StudentSkill.objects.create(
            student_profile=self.student_profile, skill=self.python, level=2
        )
        StudentSkill.objects.create(
            student_profile=self.student_profile, skill=self.sql, level=2
        )
    ...
    def test_recommendations_sorted_descending(self):
        ...
        VacancySkill.objects.create(
            vacancy=low, skill=self.sql, required_level=5, weight=1, is_critical=False
        )
        StudentSkill.objects.create(
            student_profile=self.student_profile, skill=self.python, level=3
        )
        StudentSkill.objects.create(
            student_profile=self.student_profile, skill=self.sql, level=1
        )
...
class RecommendationPageTests(TestCase):
    ...
    def setUp(self):
        ...
        StudentSkill.objects.create(
            student_profile=self.student_profile, skill=self.skill, level=3
        )
...
class SkillWheelTests(TestCase):
    ...
    def setUp(self):
        self.student_user = get_user_model().objects.create_user(
            email="wheel.student@example.com", password="StrongPass123!", role="student"
        )
        self.student_profile = StudentProfile.objects.create(
            user=self.student_user,
        ...
        self.python = Skill.objects.create(
            name="Python", category=Skill.Category.BACKEND
        )
        self.react = Skill.objects.create(
            name="React", category=Skill.Category.FRONTEND
        )
    ...
    def test_compute_skill_wheel_returns_categories(self):
        StudentSkill.objects.create(
            student_profile=self.student_profile, skill=self.python, level=4
        )
        StudentSkill.objects.create(
            student_profile=self.student_profile, skill=self.react, level=4
        )
        StudentSkill.objects.create(
            student_profile=self.student_profile, skill=self.git, level=5
        )
    ...
    def test_next_skill_recommendations_excludes_existing_student_skills(self):
        ...
        extra_skill = Skill.objects.create(
            name="Docker", category=Skill.Category.DEVOPS
        )
        StudentSkill.objects.create(
            student_profile=self.student_profile, skill=self.react, level=4
        )
