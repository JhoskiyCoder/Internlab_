...
def _get_market_requirements_by_vacancy() -> list[list[VacancySkill]]:
    published_vacancies = Vacancy.objects.filter(
        status=Vacancy.Status.PUBLISHED
    ).prefetch_related("required_skills__skill")
    return [list(vacancy.required_skills.all()) for vacancy in published_vacancies]


def _get_primary_market_categories(
    requirements_by_vacancy: list[list[VacancySkill]],
) -> list[str | None]:
    ...
    for requirements in requirements_by_vacancy:
        ...
        primary_categories.append(
            best_category if category_weights[best_category] else None
        )
    return primary_categories


def compute_skill_wheel(student_profile):
    student_skill_items = StudentSkill.objects.filter(
        student_profile=student_profile
    ).select_related("skill")
    student_skills_by_category = {
        category_name: [] for category_name in SKILL_WHEEL_CATEGORY_SKILLS
    }
    ...
    student_skills_total = sum(
        (len(skill_names) for skill_names in student_skills_by_category.values())
    )
    market_requirements_by_vacancy = _get_market_requirements_by_vacancy()
    primary_market_categories = _get_primary_market_categories(
        market_requirements_by_vacancy
    )
    market_skill_names = {
    ...
    for category_name, skills in SKILL_WHEEL_CATEGORY_SKILLS.items():
        normalized_skills = {
            _normalize_skill_name(skill_name): skill_name for skill_name in skills
        }
        ...
        score = (
            round(skills_matched / student_skills_total * 100)
            if student_skills_total
            else 0
        )
        market_score = (
...
def compute_next_skill_recommendations(student_profile, limit: int = 3):
    student_skill_ids = set(
        StudentSkill.objects.filter(student_profile=student_profile).values_list(
            "skill_id", flat=True
        )
    )
    published_vacancies_count = Vacancy.objects.filter(
        status=Vacancy.Status.PUBLISHED
    ).count()
    market_requirements = (
    ...
    for requirement in market_requirements:
        ...
        entry["market_signal"] += requirement.weight + (
            2 if requirement.is_critical else 0
        )
    recommendations = []
    for entry in aggregated.values():
        entry["market_signal"] += entry["vacancies_count"]
        entry["market_percent"] = (
            round(entry["vacancies_count"] / published_vacancies_count * 100)
            if published_vacancies_count
            else 0
        )
        if entry["critical_count"] >= 2 or entry["market_percent"] >= 60:
            ...
            entry["hint"] = (
                f"Нужен в {entry['market_percent']}% вакансий по вашему направлению"
            )
...
def calculate_vacancy_match(student_profile, vacancy: Vacancy) -> MatchingResult:
    ...
    student_levels = {
        ...
        for item in StudentSkill.objects.filter(
            student_profile=student_profile
        ).select_related("skill")
    }
    weighted_sum = 0.0
    ...
    for requirement in requirements:
        ...
        skill_match = (
            min(student_level / requirement.required_level, 1)
...
def get_recommended_vacancies(student_profile):
    ...
    results = [
        calculate_vacancy_match(student_profile, vacancy) for vacancy in vacancies
    ]
