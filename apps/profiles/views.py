...
def student_profile_detail(request):
    ...
    avg_skill_level = (
        round(sum((item.level for item in skills)) / skills.count(), 1)
        if skills.exists()
        else 0
    )
    ...
    if request.method == "POST":
        ...
        if form_type == "about_quick_edit":
            settings_form = StudentProfileSettingsForm(
            ...
            normalized_languages = [
                line.strip() for line in raw_languages.splitlines() if line.strip()
            ]
            ...
            if project_form.is_valid():
                ...
                return redirect(
                    f"{reverse('profiles:student_profile_detail')}?tab=profile"
                )
            ...
            project = get_object_or_404(
                StudentProject, pk=project_pk, student_profile=profile
            )
            project.delete()
    ...
    languages = _parse_languages(profile.languages) or [
        "Русский (родной)",
        "Английский (B2)",
        "Немецкий (A1)",
    ]
    completion_items = [
        {"label": "Фото профиля", "done": bool(profile.avatar)},
        {
            ...
            "done": bool(profile.full_name and profile.university and profile.course),
        },
    ...
    completion_percent = (
        int(completion_done / len(completion_items) * 100) if completion_items else 0
    )
    return render(
        request,
...
def employer_profile_detail(request):
    ...
    latest_vacancies = vacancies.annotate(
...
def employer_profile_create(request):
    ...
    return render(
        ...
        {
            "form": form,
            "title": "Создание профиля работодателя",
            "submit_text": "Создать",
        },
    )
...
def employer_profile_edit(request):
    ...
    return render(
        ...
        {
            "form": form,
            "title": "Редактирование профиля работодателя",
