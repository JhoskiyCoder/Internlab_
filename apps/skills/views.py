...
def student_skill_list(request):
    ...
    avg_level = round(sum(item.level for item in skills) / skills_count, 1) if skills_count else 0
...
def student_skill_edit(request, pk):
    ...
    if request.method == "POST":
        form = StudentSkillForm(request.POST, instance=student_skill, student_profile=profile)
        if form.is_valid():
            form.save()
...
def student_skill_delete(request, pk):
    ...
    return render(request, "skills/student_skill_confirm_delete.html", {"student_skill": student_skill})
