...
class StudentSkillFlowTests(TestCase):
    ...
    def test_student_can_add_skill(self):
        ...
        self.assertTrue(
            StudentSkill.objects.filter(
                student_profile=self.profile_1, skill=self.python_skill, level=4
            ).exists()
        )

    def test_student_cannot_add_duplicate_skill(self):
        StudentSkill.objects.create(
            student_profile=self.profile_1, skill=self.python_skill, level=3
        )
        self.client.force_login(self.student_1)
        response = self.client.post(
            reverse("skills:student_skill_add"),
        ...
        self.assertEqual(
            StudentSkill.objects.filter(
                student_profile=self.profile_1, skill=self.python_skill
            ).count(),
            1,
        )
    ...
    def test_student_can_edit_own_skill(self):
        student_skill = StudentSkill.objects.create(
            student_profile=self.profile_1, skill=self.python_skill, level=2
        )
        self.client.force_login(self.student_1)
        self.client.post(
    ...
    def test_student_can_delete_own_skill(self):
        ...
        self.client.force_login(self.student_1)
        self.client.post(
            reverse("skills:student_skill_delete", args=[student_skill.pk]), follow=True
        )
