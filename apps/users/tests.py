...
class AuthFlowTests(TestCase):
    ...
    def test_registration_creates_user_logs_in_and_redirects_student(self):
        ...
        self.assertTrue(
            get_user_model().objects.filter(email="student1@example.com").exists()
        )
        self.assertTrue(response.context["user"].is_authenticated)
        self.assertEqual(
            response.redirect_chain[-1][0], reverse("profiles:student_profile_create")
        )
    ...
    def test_registration_redirects_employer_to_employer_profile(self):
        ...
        self.assertTrue(response.context["user"].is_authenticated)
        self.assertEqual(
            response.redirect_chain[-1][0], reverse("users:employer_dashboard")
        )
